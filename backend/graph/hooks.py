...
def validate_dependencies(node_name: str, state: FinSightState):
    ...
    requirements = {
        ...
        "Report Generator Agent": ["dataset_profile", "business_analytics", "ai_insights"]
    }
    
    if node_name in requirements:
        for req in requirements[node_name]:
            if not state.get(req):
                raise ValueError(f"Dependency missing: {node_name} requires {req} in state.")
...
def after_node(node_name: str, state: FinSightState, result: Dict[str, Any], start_time: float) -> Dict[str, Any]:
    ...
    if result and "agent_logs" in result:
        for log in result["agent_logs"]:
            if log.get("status") == "FAILED":
                return on_error(node_name, Exception(log.get("message", "Unknown error")), state, critical=True)
...
def on_error(node_name: str, error: Exception, state: FinSightState, critical: bool = True) -> Dict[str, Any]:
