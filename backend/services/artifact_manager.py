...
class ArtifactManager:
    ...
    def _create_structure(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to create artifact structure for {self.workflow_id}: {e}"
            )
    ...
    def save_json(
        self, subdir: Union[str, Path], filename: str, data: Union[dict, list]
    ) -> str:
    ...
    def save_binary(
        self, subdir: Union[str, Path], filename: str, content: bytes
    ) -> str:
    ...
    def generate_manifest(self, state: Dict[str, Any]) -> str:
        ...
        logger.info(
            f"ArtifactManager successfully built workflow bundle at {self.base_dir}"
        )
        return filepath
    ...
    def bundle_results(self) -> str:
        ...
        try:
            temp_zip = shutil.make_archive(
                str(self.base_dir / "Results"), "zip", root_dir=str(self.base_dir)
            )
