...
def detect_domain_heuristics(col_names: List[str]) -> str:
    ...
    if "patient" in cols or "medical" in cols or "diagnosis" in cols:
        return "Healthcare"
    if "sku" in cols or "cart" in cols or "checkout" in cols:
        return "E-commerce"
    if "store" in cols or "pos" in cols or "inventory" in cols:
        return "Retail"
    if "account" in cols or "iban" in cols or "branch" in cols:
        return "Banking"
    if "policy" in cols or "claim" in cols or "premium" in cols:
        return "Insurance"
    if "factory" in cols or "batch" in cols or "yield" in cols:
        return "Manufacturing"
    if "sim" in cols or "data_usage" in cols or "roaming" in cols:
        return "Telecom"
    if "subscription" in cols or "mrr" in cols or "churn" in cols:
        return "SaaS"
