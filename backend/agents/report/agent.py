...
def report_node(state: FinSightState) -> Dict[str, Any]:
    ...
    try:
        ...
        downloads = {
            "PDF Report": (
                f"../reports/{Path(final_pdf_path).name}" if final_pdf_path else None
            ),
            "Markdown Report": f"../reports/{Path(md_path).name}" if md_path else None,
            "Analytics JSON": (
                "../analytics/analytics.json"
                if (artifact_mgr.get_analytics_dir() / "analytics.json").exists()
                else None
            ),
            "Validation JSON": (
                "../validation/validation.json"
                if (artifact_mgr.get_validation_dir() / "validation.json").exists()
                else None
            ),
        ...
        log = {
            ...
            "warnings": (
                []
                if success
                else ["PDF generation skipped due to missing dependencies."]
            ),
        }
