...
def calculate_quality(profiles: List[ColumnProfile], total_rows: int) -> QualityScore:
    ...
    if not profiles or total_rows == 0:
        return QualityScore(
            missing_value_score=0.0, duplicate_score=0.0, outlier_score=0.0,
            consistency_score=0.0, completeness_score=0.0, overall_score=0.0,
    ...
    avg_dupe_pct = sum((p.duplicate_values / total_rows * 100) for p in profiles if p.cardinality == "High") / max(sum(1 for p in profiles if p.cardinality == "High"), 1)
    duplicate_score = max(0.0, 100.0 - (avg_dupe_pct / 2)) # Div 2 to be lenient on column-level dupes
    ...
    overall = (missing_score * 0.3) + (duplicate_score * 0.2) + (outlier_score * 0.2) + (consistency_score * 0.15) + (completeness_score * 0.15)
