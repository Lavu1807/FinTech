...
def infer_semantic_type(series: pd.Series, name: str) -> str:
    ...
    if "phone" in name_lower or "mobile" in name_lower:
        return "Phone"
    if "ssn" in name_lower or "social security" in name_lower:
        return "SSN"
    if "pan" in name_lower and "company" not in name_lower:
        return "PAN"
    ...
    if "ifsc" in name_lower:
        return "IFSC"
    if (
        "credit card" in name_lower
        or "card number" in name_lower
        or "cc_" in name_lower
    ):
        return "Credit Card"
    if "account number" in name_lower or "acct" in name_lower:
        return "Account Number"
    if "medical" in name_lower or "health" in name_lower or "diagnosis" in name_lower:
        return "Medical Information"
    if "salary" in name_lower or "income" in name_lower or "wage" in name_lower:
        return "Salary"
    if "dob" in name_lower or "birth" in name_lower:
        return "Birth Date"
    ...
    if pd.api.types.is_bool_dtype(series) or (
        series.dropna().isin([0, 1, "0", "1", "True", "False", True, False]).all()
    ...
    if pd.api.types.is_numeric_dtype(series):
        if (
            ...
            or "cost" in name_lower
            or "currency" in name_lower
        ):
            return "Currency"
        if (
            "percent" in name_lower
            or "rate" in name_lower
            or series.dropna().between(0, 1).mean() > 0.9
        ):
            return "Percentage"
        return "Numerical"
...
def profile_categorical(series: pd.Series) -> Dict[str, Any]:
    ...
    imbalance_ratio = (
        float(top_10.iloc[0] / max(top_10.iloc[-1], 1)) if len(top_10) > 1 else 1.0
    )

    return {
        "top_values": {str(k): int(v) for k, v in top_values.items()},
        "frequency": {
            str(k): round(float(v / len(s)), 4) for k, v in top_values.items()
        },
    ...
    for col in df.columns:
        try:
            ...
            profile = ColumnProfile(
                ...
                duplicate_values=(
                    int(total_rows - nunique - null_count)
                    if total_rows > nunique
                    else 0
                ),
