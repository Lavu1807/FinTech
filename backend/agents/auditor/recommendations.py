...
def detect_risks(profiles: List[ColumnProfile]) -> List[DatasetRisk]:
    ...
    pii_types = {"Email", "Phone", "SSN", "PAN", "Aadhaar", "Passport", "IBAN", "IFSC", 
                 "Credit Card", "Account Number", "Medical Information", "Salary", "Birth Date"}
...
def generate_recommendations(profiles: List[ColumnProfile]) -> List[CleaningRecommendation]:
    ...
    for p in profiles:
        ...
        if p.missing_percentage > 50:
            ...
            action = "Median Imputation" if p.inferred_semantic_type in ["Numerical", "Currency", "Percentage"] else "Mode Imputation"
            recommendations.append(CleaningRecommendation(
                column=p.name,
        ...
        if p.unique_percentage > 90 and p.inferred_semantic_type in ["Text", "Categorical"]:
            recommendations.append(CleaningRecommendation(
                column=p.name,
