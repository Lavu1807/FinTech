...
class TrendValidator(BaseValidator):
    def validate(self, insight: str, score: ValidationScore, state: FinSightState) -> ValidationScore:
        ...
        if "increas" in insight_lower and "decreas" in actual_trend:
            ...
            score.validation_notes.append(f"Insight claims increase, but actual trend is '{actual_trend}'.")
            
        if "decreas" in insight_lower and "increas" in actual_trend:
            score.consistency_score = max(0.0, score.consistency_score - 0.4)
            score.validation_notes.append(f"Insight claims decrease, but actual trend is '{actual_trend}'.")
        ...
        if "spike" in insight_lower or "jump" in insight_lower:
            ...
            if not spikes:
                ...
                score.validation_notes.append("Insight claims a spike, but no statistical spikes were detected.")
                
        # 3. Comparative claim validation ("higher than", "outperforms", "best")
        comparative_keywords = ["higher than", "lower than", "outperform", "best", "worst", "leading", "lagging"]
