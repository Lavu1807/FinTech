...
def export_validation_report(scores: List[ValidationScore], workflow_id: str) -> str:
    ...
    results_path = artifact_mgr.save_json(
        "validation", "validation_results.json", [s.model_dump() for s in scores]
    )

    # 2. Validation summary
    summary_path = artifact_mgr.save_json(
        "validation", "validation_summary.json", summary.model_dump()
    )

    # 3. Unsupported claims
    unsupported_path = artifact_mgr.save_json(
        "validation", "unsupported_claims.json", summary.unsupported_claims
    )
