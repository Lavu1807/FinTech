...
class ChartStrategy(ABC):
    ...
    @abstractmethod
    def generate(
        self, df: pd.DataFrame, analysis_type: str, charts_dir: str
    ...
    def generate(
        ...
        if len(numerics) >= 2:
            ...
            try:
                ...
                sns.scatterplot(
                    data=df, x=numerics[0], y=numerics[1], alpha=0.6, edgecolor=None
                )
                plt.title(title, fontweight="bold")
                plt.grid(True, linestyle="--", alpha=0.7)
                plt.savefig(png_path, dpi=300)
...
class ChartRegistry:
    ...
    def generate_all(
        self, df: pd.DataFrame, analysis_plan: List[str]
    ) -> tuple[List[ChartMetadata], int]:
        charts = []
        skipped_count = 0

        # If no plan is provided, fallback to generating everything possible
        plan_to_use = (
            [a.lower() for a in analysis_plan]
            if analysis_plan
            else ["trend", "segment", "correlat", "distribut"]
        )
        ...
        for strategy in self._strategies:
            ...
            matching_analyses = [
                a
                for a in plan_to_use
                if any(t in a or a in t for t in strategy_targets)
            ]
        ...
        try:
            self.artifact_mgr.save_json(
                "charts", "chart_manifest.json", [c.model_dump() for c in charts]
            )
        except Exception:
            pass
