...
def export_to_tableau(df: pd.DataFrame, state: Dict[str, Any], result: VisualizationResult, workflow_id: str) -> str:
    ...
    try:
        ...
        if kpis:
            pd.DataFrame(list(kpis.items()), columns=["KPI_Name", "Value"]).to_csv(tableau_dir / "kpis.csv", index=False)
            
        trends = analytics.get("trend_analysis", {})
        if trends:
            pd.DataFrame(list(trends.items()), columns=["Trend_Metric", "Value"]).to_csv(tableau_dir / "trend_analysis.csv", index=False)
            
        # 3. Export Chart Metadata
        artifact_mgr.save_json("tableau", "chart_metadata.json", [c.model_dump() for c in result.charts])
        ...
        dashboard_layout = {
            ...
            "recommended_sections": [
                {"section": "Overview", "charts": [c.chart_id for c in result.charts if c.chart_type in ["Pie Chart", "Bar Chart"]]},
                {"section": "Deep Dive", "charts": [c.chart_id for c in result.charts if c.chart_type not in ["Pie Chart", "Bar Chart"]]}
            ],
