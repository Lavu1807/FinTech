...
def analytics_node(state: FinSightState) -> Dict[str, Any]:
    ...
    if df is None:
        ...
        logger.warning(f"{agent_name}: 'cleaned_dataframe' missing. Falling back to 'raw_dataframe'.")
    ...
    segments = analyze_segments(df) if (run_all or "segment" in plan_str or "compar" in plan_str) else {}
    stats = compute_statistics(df) if (run_all or "stat" in plan_str or "correl" in plan_str) else {}
    anomalies = detect_anomalies(df) if (run_all or "anomal" in plan_str or "outlier" in plan_str) else []
    
    # Visualization Recommendations
    vis_recs = []
    if trends:
        vis_recs.append("Recommend visualizing trends over time using line charts to track growth.")
    if segments:
        top_keys = list(segments.keys())[:3]
        vis_recs.append(f"Recommend visualizing segmentation for key dimensions ({', '.join(top_keys)}) using bar charts.")
    if anomalies:
        vis_recs.append("Recommend highlighting detected anomalies using scatter plots.")
