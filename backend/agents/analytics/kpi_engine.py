...
class GenericKPIStrategy(KPIStrategy):
    def calculate(self, df: pd.DataFrame) -> Dict[str, Any]:
        missing_pct = (
            float(df.isnull().sum().sum() / (df.shape[0] * df.shape[1]) * 100)
            if df.shape[0] > 0
            else 0
        )
        duplicate_pct = (
            float(df.duplicated().sum() / df.shape[0] * 100) if df.shape[0] > 0 else 0
        )
        return {
        ...
        for col in cats:
            ...
            res["Categorical Stats"][col] = {
                ...
                "Top Category Frequency": (
                    int(val_counts.iloc[0]) if len(val_counts) > 0 else 0
                ),
                "Top Category %": (
                    float(val_counts.iloc[0] / len(s) * 100)
                    if len(s) > 0 and len(val_counts) > 0
                    else 0.0
                ),
            }
        return res
    ...
    def calculate(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if cust_col:
            ...
            if rev_col and unique_cust > 0:
                res["Average Spend per Customer"] = float(
                    res.get("Total Revenue", 0) / unique_cust
                )
