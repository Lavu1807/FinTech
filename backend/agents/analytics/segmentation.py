...
def analyze_segments(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in cat_cols:
        ...
        if len(grouped) > 1 and grouped.iloc[1] > 0:
            top_vs_next = float(
                ((grouped.iloc[0] - grouped.iloc[1]) / grouped.iloc[1]) * 100
            )

        segments[col] = {
            "Top Performers": {str(k): float(v) for k, v in top.to_dict().items()},
            "Bottom Performers": {
                str(k): float(v) for k, v in bottom.to_dict().items()
            },
            "Largest Contributor %": (
                float((grouped.iloc[0] / total_val) * 100) if total_val > 0 else 0.0
            ),
            "Smallest Contributor %": (
                float((grouped.iloc[-1] / total_val) * 100) if total_val > 0 else 0.0
            ),
