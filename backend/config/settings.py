...
class Settings(BaseSettings):
    ...
    APP_ENV: AppEnvironment = Field(default=AppEnvironment.DEVELOPMENT, description="Application Environment")
    ...
    POSTGRES_PASSWORD: SecretStr = Field(default=SecretStr("password"), description="PostgreSQL password")
    POSTGRES_DB: str = Field(default="finsight_db", description="PostgreSQL database name")
    POSTGRES_PORT: int = Field(default=5432, description="PostgreSQL port")

    # LLM Settings
    GEMINI_API_KEY: Optional[SecretStr] = Field(default=None, description="Gemini API Key")
    MISTRAL_API_KEY: Optional[SecretStr] = Field(default=None, description="Mistral API Key")
    LLM_PROVIDER: str = Field(default="mistral", description="The LLM Provider to use")

    # Workflow Settings
    EXPORTS_DIR: str = Field(default="backend/exports", description="Root directory for generated exports")
    ...
    def sqlalchemy_database_uri(self) -> str:
        ...
        """
        import urllib.parse
        encoded_password = urllib.parse.quote_plus(self.POSTGRES_PASSWORD.get_secret_value())
