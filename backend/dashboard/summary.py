...
def build_summary(state: FinSightState) -> WorkflowSummary:
    ...
    failed_agents = [log.get("agent_name", "") for log in agent_logs if log.get("status") == "FAILED"]
...
def build_metrics(state: FinSightState, agent_executions: List[AgentExecution]) -> ExecutionMetrics:
    execution_metadata = state.get("execution_metadata", {})
    ...
    average_agent_runtime = total_runtime / len(agent_executions) if agent_executions else 0.0
    ...
    most_expensive = max(agents_with_cost, key=lambda a: a.estimated_cost) if agents_with_cost else None
    
    total_tokens = sum(a.estimated_tokens for a in agent_executions)
    average_tokens_per_agent = total_tokens / len(agent_executions) if agent_executions else 0.0
