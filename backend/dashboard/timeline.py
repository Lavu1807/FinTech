...
def build_timeline(state: FinSightState) -> List[TimelineEvent]:
    ...
    if start_time:
        events.append(TimelineEvent(timestamp=start_time, event_name="Dataset Uploaded"))
    ...
    for log in agent_logs:
        ...
        if last_timestamp and timestamp:
            events.append(TimelineEvent(timestamp=last_timestamp, event_name=f"{agent_name} Started"))
            
        if timestamp:
            events.append(TimelineEvent(timestamp=timestamp, event_name=f"{agent_name} Finished"))
