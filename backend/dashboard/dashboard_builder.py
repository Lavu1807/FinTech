...
def build_dashboard(state: FinSightState) -> None:
    ...
    try:
        ...
        artifact_mgr.save_json(
            "dashboard", "timeline.json", [t.model_dump() for t in dashboard.timeline]
        )
        artifact_mgr.save_json(
            "dashboard", "execution_summary.json", dashboard.summary.model_dump()
        )
        artifact_mgr.save_json(
            "dashboard", "execution_metrics.json", dashboard.metrics.model_dump()
        )

        logger.info(
            f"Dashboard metadata successfully exported to {artifact_mgr.get_dashboard_dir()}"
        )
