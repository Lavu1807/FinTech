...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        logger.info(f"Starting {settings.PROJECT_NAME} in {settings.APP_ENV.value} mode.")
        ...
        if settings.APP_ENV == "production":
            ...
            if provider == "gemini" and not settings.GEMINI_API_KEY:
                logger.error("CRITICAL: GEMINI_API_KEY is missing in production environment!")
                raise RuntimeError("Missing GEMINI_API_KEY in production.")
            elif provider == "mistral" and not settings.MISTRAL_API_KEY:
                logger.error("CRITICAL: MISTRAL_API_KEY is missing in production environment!")
