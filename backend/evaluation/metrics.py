...
def calculate_metrics(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Confidence metrics
    avg_planner_confidence = sum([r.get("planner_confidence", 0.0) for r in results]) / total_runs
    avg_validation_confidence = sum([r.get("validation_confidence", 0.0) for r in results]) / total_runs
    avg_hallucination_rate = sum([r.get("hallucination_rate", 0.0) for r in results]) / total_runs
    avg_evidence_score = sum([r.get("evidence_score", 0.0) for r in results]) / total_runs
