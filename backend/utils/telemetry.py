...
class ExecutionTelemetry:
    ...
    @classmethod
    def generate_failure_report(cls, state: FinSightState, error: Exception, node_name: str):
        ...
        report = {
            ...
            "input_state_summary": {
                ...
                "completed_agents": state.get("workflow_tracking", {}).get("completed_agents", [])
            },
            "recommended_fix": "Review the exception stack trace and verify that upstream dependencies were generated correctly."
        }
        cls._save_json(workflow_id, f"failure_report_{node_name.replace(' ', '_').lower()}.json", report)
    ...
    def generate_workflow_exports(cls, state: FinSightState):
        ...
        if not graph:
            graph = ["START"] + state.get("workflow_tracking", {}).get("completed_agents", []) + ["END"]
    ...
    def _generate_agent_performance(cls, state: FinSightState, workflow_id: str):
        ...
        slowest = max(agent_times.items(), key=lambda x: x[1])[0] if agent_times else None
        fastest = min(agent_times.items(), key=lambda x: x[1])[0] if agent_times else None
    ...
    def _generate_dataset_fingerprint(cls, state: FinSightState, workflow_id: str):
        ...
        data = {
            ...
            "column_names": list(state.get("dataset_profile", {}).get("columns", {}).keys()) if state.get("dataset_profile", {}).get("columns") else [],
            ...
            "planner_confidence": state.get("workflow_tracking", {}).get("planner_confidence", 0.0),
    ...
    def _generate_workflow_explanation(cls, state: FinSightState, workflow_id: str):
        ...
        data = {
            ...
            "why_charts_were_generated": [{"chart": c.get("chart_title"), "reason": c.get("description")} for c in viz] if isinstance(viz, list) else [],
            "why_recommendations_made": insights.get("executive_summary", "N/A"),
            "why_validation_passed": [{"insight": v.get("insight"), "status": v.get("status"), "notes": v.get("validation_notes")} for v in val] if isinstance(val, list) else []
        }
    ...
    def _generate_workflow_summary(cls, state: FinSightState, workflow_id: str):
        ...
        success = "COMPLETED" if "error_messages" not in state or not state["error_messages"] else "FAILED"
        ...
        data = {
            ...
            "reports_generated": 3 if state.get("reports", {}).get("markdown_path") else 0,
            "validation_status": all(v.get("status") == "PASS" for v in val) if isinstance(val, list) and val else "Unknown",
