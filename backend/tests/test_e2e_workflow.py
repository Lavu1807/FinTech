...
def test_end_to_end_workflow():
    ...
    analyze_response = client.post(
        f"{settings.API_V1_STR}/analyze", json={"filename": dataset_name}
    )
    ...
    assert (
        workflow_dir / "dashboard" / "Dashboard.html"
    ...
    assert (
        workflow_dir / "logs" / "agent_trace.json"
    ).exists(), "agent_trace.json not found."
    assert (
        workflow_dir / "analytics" / "business_kpis.json"
    ).exists(), "business_kpis.json not found."
    assert (
        workflow_dir / "validation" / "validation_results.json"
    ).exists(), "validation_results.json not found."
