        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for folder_name in target_folders:
                ...
                if not folder_path.exists():
                    logger.info(
                        f"ZipExporter: Skipping missing folder '{folder_name}'."
                    )
                    continue
                ...
                for root, _, files in os.walk(folder_path):
                    for file in files:
                        ...
                        if not file_path.exists():
                            logger.warning(
                                f"ZipExporter: File {file_path} vanished, skipping."
                            )
                            continue
            ...
            for item in source_dir.iterdir():
                if (
                    item.is_file() and item.name != "Results.zip"
                ):  # Don't package old zips
                    arcname = item.relative_to(source_dir)
                    zipf.write(item, arcname)
                    packaged_files.append(str(arcname))

        logger.info(
            f"ZipExporter successfully packaged {len(packaged_files)} files to {zip_path}"
        )
        return zip_path
