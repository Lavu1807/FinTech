...
def build_dataset_overview(state: Dict[str, Any]) -> list:
    ...
    if not profile:
        story.append(
            Paragraph("Dataset profile not available.", styles["BodyTextCustom"])
        )
        story.append(PageBreak())
        return story
...
def build_data_quality(state: Dict[str, Any]) -> list:
    ...
    if not scores:
        story.append(
            Paragraph("Quality assessment not available.", styles["BodyTextCustom"])
        )
        story.append(PageBreak())
        return story
...
def build_insights_and_risks(state: Dict[str, Any]) -> list:
    ...
    if recommendations:
        ...
        story.append(
            Paragraph("No recommendations provided.", styles["BodyTextCustom"])
        )
...
def build_validation_summary(state: Dict[str, Any]) -> list:
    ...
    if not validation:
        story.append(
            Paragraph("No validation data available.", styles["BodyTextCustom"])
        )
        story.append(PageBreak())
        return story
    ...
    data = [["Metric", "Value"]]
    data.append(
        ["Overall Status", "PASSED" if validation.get("is_valid") else "FLAGGED"]
    )
    data.append(
        ["Hallucination Score", f"{validation.get('hallucination_score', 0)*100:.1f}%"]
    )
...
def build_appendix(state: Dict[str, Any]) -> list:
    ...
    data.append(
        ["Execution Time", f"{meta.get('total_execution_time', 0):.2f} seconds"]
    )
    ...
    for agent in state.get("agent_logs", []):
        ...
        story.append(
            Paragraph(
                f"<b>{name}</b> [{status}] - {time_str}", styles["BodyTextCustom"]
            )
        )
