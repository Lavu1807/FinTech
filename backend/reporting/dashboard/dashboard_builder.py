...
def generate_dashboard(state: FinSightState, downloads: Dict[str, str] = None) -> str:
    ...
    business_domain = state.get("dataset_info", {}).get(
    ...
    chart_paths = [
        chart.get("file_path") for chart in charts_metadata if chart.get("file_path")
    ]
...
def export_html_dashboard(
    state: FinSightState, downloads: Dict[str, str] = None, workflow_id: str = None
) -> str:
