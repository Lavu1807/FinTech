...
def extract_agent_summaries(state: FinSightState) -> Dict[str, Any]:
    ...
    if isinstance(validation_results, list):
        summaries["validation_checks"] = [
            ...
            for v in validation_results if isinstance(v, dict)
        ]
    else:
        summaries["validation_checks"] = []
    ...
    if isinstance(charts, list):
        summaries["visualization_summary"] = [
            ...
            for c in charts if isinstance(c, dict)
        ]
    else:
        summaries["visualization_summary"] = []
    ...
    if isinstance(summaries["executive_summary"], dict):
        summaries["executive_summary"] = summaries["executive_summary"].get("executive_summary", "")
