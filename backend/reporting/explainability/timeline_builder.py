...
def build_workflow_timeline(state: FinSightState) -> Dict[str, Any]:
    ...
    if not execution_graph:
        execution_graph = ["START"] + state.get("workflow_tracking", {}).get("completed_agents", []) + ["END"]

    return {
        "execution_summary": {
            "total_execution_time": meta.get("total_execution_time", sum(agent_times.values())),
            ...
            "total_estimated_cost": max(meta.get("estimated_llm_cost", 0.0), total_cost),
        },
