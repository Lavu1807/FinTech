...
def _load_workflow_json(workflow_id: str, subfolder: str, filename: str) -> dict:
    ...
    filepath = Path(settings.EXPORTS_DIR) / "workflows" / workflow_id / subfolder / filename
    ...
    if not filepath.exists():
        raise HTTPException(status_code=404, detail=f"{filename} not found for workflow {workflow_id}")
    try:
