...
def _execute_workflow_task(
    workflow_id: str, session_id: str, file_path: Path, filename: str
):
    ...
    try:
        ...
        final_state = run_workflow(
            df, filename=filename, session_id=session_id, workflow_id=workflow_id
        )

        # Update status on completion
        _WORKFLOW_STATUS[workflow_id] = {
            "status": final_state.get("execution_metadata", {}).get(
...
async def analyze_dataset(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    if not file_path.exists():
        raise DatasetValidationError(
            f"Dataset {safe_filename} not found. Please upload first."
        )
    ...
    background_tasks.add_task(
        _execute_workflow_task, workflow_id, session_id, file_path, request.filename
    )

    logger.info(
        f"Started analysis for {request.filename} with workflow_id {workflow_id}"
    )
